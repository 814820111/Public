...
//            }
    ...
    public function addschool_post(){
        if(IS_POST){
            ...
            if($this->school_model->where(array("school_name"=>$school_name))->find())
            {
                $this->error("该学校已经存在,请勿重复添加!");
            }
            ...
            $this->add_school_column($schoolid);
    ...
    //添加微网站菜单
    public function  add_school_column($schoolid){
//        $schoolid=$schoolid;
        $data[0] = array(
            "name"=>"我的学校",
            "status"=>1,
            ...
            "sortid"=>10,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img01.png",
            "type"=>1,
            "column_type"=>2,
        ...
        $data[1] = array(
            ...
            "sortid"=>9,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img03.png",
            "type"=>1,
            "column_type"=>1,
            "action"=>""
        );
        $data[2] = array(
            ...
            "sortid"=>8,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img02.png",
            "type"=>1,
            "column_type"=>1,
            "action"=>""
        );
        $data[3] = array(
            ...
            "sortid"=>7,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img07.png",
            "type"=>1,
            "column_type"=>1,
            "action"=>""
        );
        $data[4] = array(
            ...
            "sortid"=>4,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img04.png",
            "type"=>1,
            "column_type"=>1,
            "action"=>""
        );
        $data[5] = array(
            ...
            "sortid"=>6,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img05.png",
            "type"=>1,
            "column_type"=>2,
            "action"=>"JobManage"
        );
        $data[6] = array(
            ...
            "sortid"=>5,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img08.png",
            "type"=>1,
            "column_type"=>2,
            "action"=>"Online_enroll"
        ...
        $data[7] = array(
            ...
            "sortid"=>1,
            "schoolid"=>$schoolid,
            "icon"=>"http://ow3hm6y11.bkt.clouddn.com/img06.png",
            "type"=>1,
            "column_type"=>2,
            "action"=>"mailbox"
...
//        echo "学校审核详情";
    ...
    public function classmanage(){
        ...
        //查询出这个学校年级段
        $school_grade = M("school_grade")->where(array("schoolid"=>$schoolid))->select();
...
//    }
    ...
    public function addclass(){
        ...
        $grade=I('grade');
        $classname = $this->getclassname($schoolid,$grade);
        $this->assign("grade_type",I('grade_type'));
    ...
    private function getclassname($schoolid,$grade){
        $classname='';
        if($grade){
            $num=M("SchoolClass")->where("schoolid = '$schoolid' and grade = '$grade' ")->count();
            $school_gradename=M("school_grade")->where("schoolid = '$schoolid' and gradeid = '$grade' ")->getField("name");
            $nums=$num+1;
            $arr_n = array("零","一","二","三","四","五","六","七","八","九","十","十一","十二","十三","十四","十五","十六","十七","十八","十九","二十");
            $classname=$school_gradename.$arr_n[$nums].'班';
        }
    ...
    public function addclass_post(){
        ...
        if($this->class_model->where(array("schoolid"=>$schoolid,"classname"=>$classname))->find())
        {
            $this->error('该班级已经存在,请勿错误添加!');
        }
        ...
        if($schoolid){
            ...
            if($classid){
                $url = U('addclass',['schoolid'=>$schoolid,'schoolname'=>$schoolname,'grade_type'=>$grade_type,'grade'=>$gradename,'classname'=>$classname]);
                $this->success("保存成功！",$url,3);
            }else{
                $this->error('数据传入失败！');
            }
        }else{
    ...
    //批量导入班级
    public function addclass_array(){
        //添加班级
        $schoolid = intval($_GET['schoolid']);
        if(empty($schoolid)){
            $this->error("未获取到学校id");
        ...
        $this->assign("info",I("info"));
        $this->assign("allcount",I("row"));
        $this->assign("successcount",I("successcount"));
        $this->display();
    }
    public function addclass_array_post(){
        ...
        $gradearray = $this->array_column($Result, 'gradeid', 'name');
        unset($Result);
        $info='';$row=1;
        if($highestColumn == "C") {

            foreach ($ExamPaper_arr as $key => $value) {
                $gardname = trim($value['B']);
                $classname = trim($value['C']);
                if(empty($gardname) || empty($gradearray[$gardname])) {
                    $info .="第".$row."行年级不存在。";
                    $row++;
                    continue;
                }
               ...
               $data=array();
                $data['schoolid']=$schoolid;
                $data['classname']=$classname;
                $data['grade']=$gradearray[$gardname];
                $data['type']=1;
                $data['create_time']=time();
                // var_dump($data);
                $classid=$this->class_model->add($data);
        ...
        $url = U('addclass_array',['schoolid'=>$schoolid,'schoolname'=>$schoolnamearr[0],'row'=>$row-1,"successcount"=>$successcount,"info"=>$info]);
        $this->success("上传成功！",$url,3);
    }
...
//        echo "黑名单管理";
    ...
    {
      $classid = implode(I("classid"),",");

      $where['id'] = array("in",$classid);

      $school_class = $this->class_model->order("id ASC")->where($where)->select();

      if($school_class)
      {
          $res = $this->format_ret(1,$school_class);
      }else{
          $res = $this->format_ret(0,"no class");
      }
    ...
    public function edit_up_class()
    {
        ...
        //班级名称
        $up_class = I("up_class");
        $is_show_class = $this->is_show_class($up_class,$schoolid);

        if(!$is_show_class['status'])
        {
            $res = $this->format_ret(0,$is_show_class['classname']);
            echo json_encode($res);
            exit();
        }

        //将两个数组合并
        $class_res = array_combine($classid,$up_class);
       //先这么写以后优化
        foreach($class_res as $oldclass=>$classname)
        {
            //查询出目标班级
            $new_classid = M("school_class")->where(array("schoolid"=>$schoolid,"classname"=>$classname))->getField("id");
            //将student_info表里的classid修改
            M("student_info")->where(array("schoollid"=>$schoolid,"classid"=>$oldclass))->save(array("classid"=>$new_classid));
            //将班级学生关系表修改
            M("class_relationship")->where(array("schoolid"=>$schoolid,"classid"=>$oldclass))->save(array("classid"=>$new_classid));
            //将卡号表修改
            M("student_card")->where(array("schoolid"=>$schoolid,"classId"=>$oldclass))->save(array("classId"=>$new_classid));
        }

        $res = $this->format_ret(1,"升班成功");
        echo json_encode($res);
        exit();
    }

    private function is_show_class($class,$schoolid)
    {
        ...
        if($class)
        {
            ...
            {
                $new_class = M("school_class")->where(array("schoolid"=>$schoolid,"classname"=>$val))->find();
                if(!$new_class)
                {
                    return array("status"=>false,"classname"=>$val);
                }
            }
    ...
    public function edit_up_grade()
    {
        ...
        $schoolid = I("schoolid");
         ...
         $start_class = M("school_class")->where(array("schoolid"=>$schoolid,"grade"=>$gradeid))->select();

         $up_grade_id = M("school_grade")->where(array("schoolid"=>$schoolid,"name"=>$up_grade))->getField("gradeid");
         $end_class = M("school_class")->where(array("schoolid"=>$schoolid,"grade"=>$up_grade_id))->select();

         if(count($start_class)>count($end_class))
         {
             $res = $this->format_ret(0,"起始班级和目标班级不相同,请将目标班级补充完整!");
             echo json_encode($res);
             exit();
         }
         $array = array();
         foreach($start_class as $key=>$val)
         {
             $array[$val['id']] = $end_class[$key]['id'];
         }

        foreach($array as $key=>$val)
        {
            //将student_info表里的classid修改
            M("student_info")->where(array("schoollid"=>$schoolid,"classid"=>$key))->save(array("classid"=>$val));
            //将班级学生关系表修改
            M("class_relationship")->where(array("schoolid"=>$schoolid,"classid"=>$key))->save(array("classid"=>$val));
            //将卡号表修改
            M("student_card")->where(array("schoolid"=>$schoolid,"classId"=>$key))->save(array("classId"=>$val));
        }
        $res = $this->format_ret(1,"升班成功");
        echo json_encode($res);
        exit();
