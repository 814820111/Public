...
class TagLibSpadmin extends TagLib {
    ...
    protected $tags = array(
        ...
        'teachertpl' => array("attr" => "file", "close" => 0),
    );
    ...
    public function _teachertpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = DIRECTORY_SEPARATOR . "Teacher" .  DIRECTORY_SEPARATOR . $tag['file'];
        } else {
            $file_path = DIRECTORY_SEPARATOR . $file[0] . DIRECTORY_SEPARATOR . "Tpl" . DIRECTORY_SEPARATOR . $file[1] . DIRECTORY_SEPARATOR . $file[2];
        }
        //模板路径
        $TemplatePath =  C("SP_TEACHER_TMPL_PATH") .C("SP_TEACHER_DEFAULT_THEME")."/". $file_path . C("TMPL_TEMPLATE_SUFFIX");
        //判断模板是否存在
        if (!file_exists_case($TemplatePath)) {
            return false;
        }
