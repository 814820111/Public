...
class MonitorWebController extends AdminbaseController
{
    ...
    public function index()
    {
        if (IS_POST) {
            ...
            $class_id = I('class_id');
            //获取该学校所有的授权列表
            $monitorInfo = M('monitor')->where("schoolid = $school_id  and classid = $class_id and type=0 and resource = 2")->select();
        } else {
            //获取所有的授权列表
            $monitorInfo = M('monitor')->alias("m")->join("wxt_monitor_channel_time as t on m.id = t.monitor_id")->where("m.type = 0 and m.resource = 2")->select();
            $count = count($monitorInfo);
            $page = $this->page($count, 15);
            $monitorInfo = M('monitor')->alias("m")->join("wxt_monitor_channel_time as t on m.id = t.monitor_id")->where("m.type = 0 and m.resource = 2")->limit($page->firstRow . ',' . $page->listRows)->order('m.id desc')->field("m.*, t.start_time, t.end_time, t.days")->select();
...
//------------------增加校园视频----------------
    ...
    function camarolist()
    {
        ...
        //根据学校ID获取所有的摄像头
        $camaro = M('monitor_live')->where("school_id = {$data['schoolid']}")->select();
        if (empty($camaro)) {
            die("该学校还没有添加摄像头");
        }
        $leCheng = array();
    ...
    function add_post()
    {
        ...
        if (!empty($isTypeOne)) {
            ...
            for ($i = 0; $i < count($isTypeOne); $i++) {
                $info = M('monitor_live')->where("id = {$isTypeOne[$i]}")->find();
                $arr = array(
    ...
    public function look()
    {
        ...
        $id = I('id');
        //获取当前ID的视频数据
        $monitor = M('monitor')->where("id = $id and type = 0 and resource = 2")->field('schoolid, id')->find();
        if (!$monitor) {
            $this->error("获取失败");
        }
    ...
    {
        $id = I('id');
        $deleteResult = M('monitor')->where("id = $id and type=0 and resource = 2")->delete();
...
//            }
            ...
            if ($updateName) {
                //修改信息成功
                $this->success('修改视频信息名称成功', U('MonitorWeb/look', array('id' => $mid)));
            } else {
                die("修改视频信息名称失败");
            }
