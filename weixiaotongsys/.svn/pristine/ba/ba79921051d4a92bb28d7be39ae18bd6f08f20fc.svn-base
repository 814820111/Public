...
class SchoolNewsManageController extends TeacherbaseController {
    ...
    function _initialize() {
        ...
        $this->news_model=D("Common/schoolnews");
    }
...
//        $where= join(" and ", $where_ands);
    ...
    $where["schoolid"] = $_SESSION["schoolid"] ;
        $count=$this->news_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->news_model
        ...
        foreach ($posts as &$news) {
            ...
            ->alias("school")
            ->join("wxt_schoolnews news")
            ->where(array("school.schoolid"=>$news['schoolid']))
            ->field('school_name')
            ->find();

            $news["schoolname"]=$schoolname["school_name"];
            unset($news);
        }
...
//------------------增加校园招聘----------------
  function add(){
      ...
      $schoolid = intval(I("get.school"));
        $this->_getTree();
        $news=$this->news_model->where("schoolid=$schoolid")->find();
...
//------------------更新校园招聘开始----------------
    public function edit(){
        ...
        $news = $this->news_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $news['smeta']=json_encode($_POST['smeta']);
            $news['post_content']=htmlspecialchars_decode($news['post_content']);
            $result=$this->news_model->where($where)->save($news);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
...
//------------------删除信息------------------------
    function delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst =  $this->news_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
