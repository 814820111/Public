...
class SchoolController extends AppBaseController {
    ...
    public function add_post(){
        ...
        $end_time=I("end");
        $recipe_time = M('cookbook')->where("date >= $begin_time and date <= $end_time and schoolid = $schoolid and grade= $grade")->select();

        if(count($recipe_time)){
            echo json_encode(array('data'=>'0','message'=>'该年级,该时间段已经发布过食谱!'));
            die();
        }

        if($grade == 0){
            $school_grade = M('school_grade')->where("schoolid = $schoolid")->order("id ASC")->select();
            foreach ($school_grade as $k =>$v) {
                $gradeid = $v['gradeid'];
                $res = M('cookbook')->where("date >= $begin_time and date <= $end_time and schoolid = $schoolid and grade= $gradeid")->select();
                if(count($res)){
                    echo json_encode(array('data'=>'0','message'=>'已有年级,该时间段已经发布过食谱,不能全校发布!'));
                    die();
                }
            }
        }
...
//        return;
        ...
        if($cook){
            echo json_encode(array('data'=>$cook,'message'=>'数据请求成功'));
        }else{
 ...
 public function getCookbookContent(){
        ...
        if(!empty($begindate) && !empty($schoolid)&& !empty($endtdate)){
            $sql = "select * from wxt_cookbook where schoolid='$schoolid' and grade='$grade' and ( date between '$begindate' and  '$endtdate')";
            $info = M()->query($sql);
...
//                    $imagesize = getimagesize("./uploads/microblog/".$pohtolist[$j]["pictureurl"]);
                    $imagesize = getimagesize("/alidata/www/weixiaotong2016/uploads/microbloghub/".$pohtolist[$j]["pictureurl"]);
	 ...
	 //获取自己发布的公告列表详情
    public function getnoticelist_details(){
        //获取参数
        $noticeid =  Intval(I('noticeid'));
        $userid = Intval(I('userid'));
        if($userid){
   ...
   public function get_receive_notice(){
        ...
        if($receiverid){
            ...
            ->where("r.receiverid=$receiverid")
             ->limit($start_page . ',' .$num)
            //->order("r.id DESC")
            ->order("r.id DESC,r.create_time asc")
            ->select();
            //foreach循环获取剩余所需信息
            foreach ($receive_info as &$notice) {
               ...
               // ->where("n.id=$noticeid")
                ->where("n.id=$noticeid and n.send_type=1")
...
//                    $imagesize = getimagesize("./uploads/microblog/".$pic[$j]["photo"]);
                    $imagesize = getimagesize("/alidata/www/weixiaotong2016/uploads/microbloghub/".$pic[$j]["photo"]);
                    $notice['pic'][$j]["picturewidth"]=$imagesize["0"];
                    $notice['pic'][$j]["pictureheight"]=$imagesize["1"];
                }
                unset($notice);
	 ...
	 //家长端 获取公告 详情
    public function get_receive_notice_details(){
        //获取参数
        $receiverid=Intval(I('receiverid'));//接收人ID
        $noticeid = Intval(I('noticeid'));//公告ID
        if($receiverid){
            $notice_model=M('notice');
            $receive_model=M('notice_receiverid');
            $pic_model = M('notice_photo');
...
/***
    ...
    public function addTask(){
        ...
        $photolist=I('photolist');
        //判断有效性
        if(!empty($userid) && !empty($reciverid)){
            $data["userid"]=$userid;
            $data["title"]=$title;
            $data["content"]=$content;
            $data["schoolid"]=$schoolid;
            ...
            if($scheduleid){
                //给自己加一条记录
                $datas['scheduleid']=$scheduleid;//任务ID
                $datas["receiverid"]=$userid;//接收人ID
                $datas["create_time"]=time();
                $datas["userid"]=$reciverid;//转派人ID
                $datas["finish"]=1;//第一步已完结
                $datas["read_time"]=time();//读取时间
                $datas["do_time"]=time();//完结时间
                $resultid = $recive_model->add($datas);
                if($resultid){
                    //添加接收记录
                    $recivedata["scheduleid"]=$scheduleid;
                    $recivedata["receiverid"]=$reciverid;//接收人ID
                    $recivedata["create_time"]=time();
                    $recivedata["previousid"]= $resultid;
                    $reciveid=$recive_model->add($recivedata);
                }
                //添加图片记录
                //截取图片字符串
     ...
     *获取我收到的待办事宜列表
     */
    public function GetMyReciveTasklist(){
        //定义数据模型
        $schedule_model=M('schedule');
        $recive_model=M('schedule_reception');
        $photo_model=M('photo');
        ...
        if($userid && $schoolid){
            ...
            $lists = $recive_model
                ...
                ->where($where)
                ->order('r.id DESC')
                ->field('s.id,s.userid,u.name,u.photo,title,content,s.status,s.create_time,r.id as rid,r.do_time,r.read_time')
                ->select();
//            echo "<pre>";
//           print_r($lists);
            foreach ($lists as &$schedule ) {
...
//                    ->select();
                    ...
                    ->where("r.scheduleid = $scheduleid ")
                    ->order('r.id ASC')
                    ->field('r.id,r.userid,r.scheduleid,r.previousid,u.name,u.photo,feedback,finish,read_time,r.do_time,r.create_time')
                    ->select();
 ...
 /**
     ...
     *获取我发布的待办事宜列表
     */
    public function GetMySendTaskList(){
        //定义数据模型
        $schedule_model=M('schedule');
        $recive_model=M('schedule_reception');
        $user_model = M('wxtuser');
    ...
    public function DoTask(){
        ...
        if($scheduleid && $userid){
            if($finish==0){
                ...
                //$updatadata["status"]=2;
                $updatadata["finish"]=1;
                $updatadata["do_time"]=time();
                $updatadata["feedback"]=$feedback;
                $previousid=$recive_model->where($updatewhere)->save($updatadata);//改变自己状态
                //添加转派记录
                $recivedata["previousid"]=intval(I('id'));
                $recivedata["scheduleid"]=$scheduleid;
                $recivedata["userid"]=$userid;
