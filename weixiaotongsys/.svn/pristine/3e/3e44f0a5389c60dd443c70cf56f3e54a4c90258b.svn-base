...
class InterestingManageController extends TeacherbaseController {
    ...
    function _initialize() {
        ...
        $this->interesting_model=D("Common/interestclass");
    }
...
//        $where= join(" and ", $where_ands);
    ...
    $where["schoolid"] = $_SESSION["schoolid"] ;
        $count=$this->interesting_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->interesting_model
        ...
        foreach ($posts as &$class) {
            ...
            ->alias("school")
            ->join("wxt_interestclass class")
            ->where(array("school.schoolid"=>$class['schoolid']))
            ->field('school_name')
            ->find();

            $class["schoolname"]=$schoolname["school_name"];
            unset($class);
        }
...
//        $school_name = $this->school_model->order(array("schoolid"=>"asc"))->select();
      $school_name = $this->school_model->where(array("schoolid"=>$_SESSION["schoolid"]))->order(array("schoolid"=>"asc"))->select();

      $schoolid = intval(I("get.school"));
        $this->_getTree();
        $class=$this->interesting_model->where("schoolid=$schoolid")->find();
    ...
    function add_post(){
        if (IS_POST) {
           ...
           //$_POST['post']['post_author']=get_current_admin_id();
            $class=I("post.post");
            $class['schoolid']=$_POST['school'];
            // $schoolname=$this->school_model->where("$school_id=schoolid")->getField('school_name');

            $class['smeta']=json_encode($_POST['smeta']);
            $class['post_content']=htmlspecialchars_decode($class['post_content']);
            $result=$this->interesting_model->add($class);
            if ($result) {
...
//         $school_name = $this->school_model->order(array("schoolid"=>"asc"))->select();
        ...
        $class = $this->interesting_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $class['smeta']=json_encode($_POST['smeta']);
            $class['post_content']=htmlspecialchars_decode($class['post_content']);
            $result=$this->interesting_model->where($where)->save($class);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
...
//------------------删除信息------------------------
    function delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst =  $this->interesting_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
