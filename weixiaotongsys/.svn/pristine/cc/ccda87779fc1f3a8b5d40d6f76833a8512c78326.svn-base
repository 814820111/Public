...
class ParentsManageController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->parents_model=D("Common/parentings");
    }
...
private  function _lists($status=1){
        ...
        $count=$this->parents_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->parents_model
        ...
        foreach ($posts as &$parents) {
             $schoolname=$this->school_model
            ->alias("school")
            ->join("wxt_parentings parents")
            ->where(array("school.schoolid"=>$parents['schoolid']))
            ->field('school_name')
            ->find();

            $parents["schoolname"]=$schoolname["school_name"];
            unset($parents);
        }
...
//------------------增加校园招聘----------------
  function add(){
        ...
        $this->_getTree();
        $parents=$this->parents_model->where("schoolid=$schoolid")->find();
    ...
    function add_post(){
        if (IS_POST) {
           ...
           //$_POST['post']['post_author']=get_current_admin_id();
            $parents=I("post.post");
            $parents['schoolid']=$_POST['school'];
            // $schoolname=$this->school_model->where("$school_id=schoolid")->getField('school_name');

            $parents['smeta']=json_encode($_POST['smeta']);
            $parents['post_content']=htmlspecialchars_decode($parents['post_content']);
            $result=$this->parents_model->add($parents);
            if ($result) {
...
//------------------更新校园招聘开始----------------
    public function edit(){
        ...
        $parents = $this->parents_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $parents['smeta']=json_encode($_POST['smeta']);
            $parents['post_content']=htmlspecialchars_decode($parents['post_content']);
            $result=$this->parents_model->where($where)->save($parents);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
