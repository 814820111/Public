...
class MicroblogController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->likes_model=D("Common/likes");
    }
...
private  function _lists($status=1){
        ...
        foreach ($posts as &$user) {
         ...
         foreach ($posts as &$photo) {
            ...
            ->where(array("microblogid"=>$photo['mid']))
            ->field('pictureurl')
            ->select();
...
//var_dump($posts);
    ...
    public function commentslist(){
        $id = intval($_GET['id']);     
        if ($id) {
              $rst = $this->comments_model
                    ->alias("c")
                    ->join("wxt_microblog_main w ON c.refid=w.mid")
                    ->field("c.id,c.refid,c.userid,c.type,c.content,c.photo,c.add_time,c.status")
                    ->where("c.refid=$id")
                    ->select();
        foreach ($rst as &$user) {
             $username=$this->wxtuser_model
...
//-------------------评论列表展示结束--------------
    function comments_delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst = $this->comments_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
    ...
    public function likeslist(){
        $id = intval($_GET['id']);   
        if ($id) {
              $rst = $this->likes_model
                    ->alias("c")
                    ->join("wxt_microblog_main w ON c.refid=w.mid")
                    ->field("c.likeid,c.refid,c.userid,c.type,c.add_time")
                    ->where("c.refid=$id")
                    ->select();
        foreach ($rst as &$user) {
             $username=$this->wxtuser_model
...
//-------------------点赞列表展示结束--------------
    function likes_delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst = $this->likes_model->where("likeid=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
