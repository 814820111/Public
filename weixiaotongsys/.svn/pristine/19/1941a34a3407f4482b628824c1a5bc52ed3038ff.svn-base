key = 1;
url_php = "index.php?g=Proscenium&m=Admin&a=uploadImg";
function upload_images(code) {
	key = code;
	if (key == 1) {
		url_php = "index.php?g=Proscenium&m=Admin&a=uploadImg";
	}
...
KindEditor.ready(function(K) {
    uploadFile(K, 'upload_logo'+key, 'img_file', url_php);
});
...
function addLaunchBody(obj, body, url){
    ...
    //'</div>'+
    '<input type="hidden"  name="url'+key+'[]" value="'+url+'">'+
    '</li>';
    $('#launch_body'+key).append(html);
    bodyAction(obj);
    //checklaunchBody();
}
