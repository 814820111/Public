...
class ParentRoleController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->school_duty_model =D("Common/school_duty_parent");
     ...
     */
        ...
        $data = M("duty_parent")->order(array("id" => "desc"))->select();
...
//        }
        $data = M('duty_parent')->where(array("id" => $id))->find();
        if (!$data) {
          $this->error("该角色不存在！");
        }
...
//    	}
    	if (IS_POST) {
    		$data = M('duty_parent')->create();
    		if ($data) {
    			if (M('duty_parent')->save($data)!==false) {
    				$this->success("修改成功！", U('ParentRole/index'));
    			} else {
    				$this->error("修改失败！");
    			}
    		} else {
    ...
    public function authorize() {
        ...
        $menu = new \Tree();
        //获取职责名称
        $role_name = M("duty_parent")->where("id = $roleid")->getField("name");
    

        $array = array();
        $priv_data=$this->auth_access_app->where("role_id=$roleid and appcatid = 1 and type = 'parent_url'")->field("status,rule_name")->select();//获取权限表数据
 ...
 $html = '';
   $html.="<tr><th style=' text-align: center;     font-size: 16px;' colspan=3 aligin=>家长端</th></tr>";
    		...
    		if (is_array($menuid) && count($menuid)>0) {
                 ...
                 if($sta)
              {
                $status = 2;
                        ...
                        unset($com_arr);
                        unset($data);
//    					$this->auth_access_app->add(array("role_id"=>$roleid,"rule_name"=>$name,'type'=>'parent_url','appcatid'=>1,"status"=>$status));
    				}
    			}
                if($is_all)
                {

                    $role_auth =  $this->auth_access_app->where(array("role_id"=>$roleid,'type'=>'parent_url',"appcatid"=>"1"))->field("rule_name")->select();

                    $this->add_syncschool_auth($roleid,$auth_array,$role_auth);
                }


                $this->auth_access_app->where(array("role_id"=>$roleid,'type'=>'parent_url',"appcatid"=>"1"))->delete();
                $this->auth_access_app->addAll($alldata);

    			$this->success("授权成功！", U("ParentRole/index"));
    		}else{
    			//当没有数据时，清除当前角色授权
    			$this->auth_access_app->where(array("role_id" => $roleid,'type'=>'parent_url',"appcatid"=>"1"))->delete();
...
//       dump($role_auth);
        ...
        foreach($school_duty as $auth)
        {
            ...
            //获取该学校的职责权限
            $school_auth  = $this->school_authaccess_app->where("role_id= $school_roleid and appcatid = 1 and type = 'parent_url'")->field("rule_name")->select();
...
//            dump($dimension);
            ...
            if($arr_del)
            {
                ...
                $where_del['type'] = 'parent_url';
                $where_del['appcatid'] = 1;
