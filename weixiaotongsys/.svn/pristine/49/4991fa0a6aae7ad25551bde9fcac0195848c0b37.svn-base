        ...
        if ($mobile != "" && $password != "") {
            ...
            if (!$userinfo) {
                ...
                $stu_real_res = M("relation_stu_user as relation")
                    ...
                    ->field("studentid,appellation,school_name,classname,user.name,class.schoolid,class.classid")
                    ->where("relation.userid = '$teacherid'")
                    ->select();
                if (count($stu_real_res) >= 1) {
                    $userinfo["parent"] = $stu_real_res;
                } else {
                    $userinfo["parent"] = array();
                }
                //调取教师信息
                $user_stu_rela = M("teacher_info as t")
                    ->join("wxt_wxtuser as w on t.teacherid = w.id")
                    ->join("wxt_teacher_class as class on class.teacherid = t.teacherid")
                    ->join("wxt_school as ws on ws.schoolid = t.schoolid")
                    ->field("w.id userid,w.phone,w.photo,w.name,w.status as userstatus ,t.schoolid,t.teacherid,t.state as teacherstate,t.bindingkey,ws.school_name,class.classid,class.type as teachertype")
                    ->where(array("t.teacherid" => $teacherid))->select();
//                $user_stu_rela = array_unique($user_stu_rela);
//                echo M("teacher_info as t")->getLastSql();
//                var_dump($user_stu_rela);
//die();
                if ($user_stu_rela) {
                    for ($i = 0; $i < count($user_stu_rela); $i++) {
                        //todo 这里的话因为数据有问题 分开写不会出问题
                        //userid schoolid
                        $tuserid = $user_stu_rela[$i]["userid"];
                        //学校id
                        $tschoolid = $user_stu_rela[$i]["schoolid"];

                        //获取班级id
                        $tclassid = M("teacher_class")->where(array("teacherid" => $tuserid, "schoolid" => $tschoolid))->find();
                        $class = $tclassid["classid"];
                         //入栈
                        $user_stu_rela[$i]["classid"] = $class;
                    }
                }
                if (count($user_stu_rela) >= 1) {
                    $userinfo["teacher_info"] = $user_stu_rela;
                } else {
                    $userinfo["teacher_info"] = array();
                }
    ...
    * 作者 ：彭书国
    */
    public function getLiveVideoListByClassId()
    {
        header('content-type: text/html; charset=utf-8');
        $classid = I("id");
        if (empty($classid)) {
   ...
   //检测版本号--家长端
    public function CheckVersionAll()
    {
        // $versionid = intval(I('versionid'));
        // $type = I('type');
        // $where['versionid'] = array('GT', $versionid);
    ...
    */
    public function GetUserRelation()
    {
        ...
        $relation_stu_return = $relation_stu->where(array("userid" => $uid))->order('preferred DESC')->select();
...
//接口  ：姜品丞
    ...
    private function uploadBase64Image($content = "")
    {
        ...
        $subContent = array_filter($subContent);
        ...
        for ($i = 0; $i < $imgCount; $i++) {
            ...
            if ($base64Img != null) {
                ...
                //获取 地址;
                $str = str_replace(array("./uploads/microblog//"), "", $imgSrc);
    ...
    private function base64_upload_image($base64, $savepath = "./uploads/microblog/")
    {
...
//        $savepath = "./uploads/microbloghub/";
        $savepath = "/alidata/www/weixiaotong2016/uploads/microbloghub/";
        //$savepath = "./uploads/microblog/";
        $resource = fopen($savepath . $jpg . ".jpg", 'w+');
        $imgSrc = $savepath . $jpg . ".jpg";//原图路径
        //将图片内容写入上述新建的文件
        fwrite($resource, $a);
        //关闭资源
        fclose($resource);
        $this->resizeImage($imgSrc,250,250,"jpg");
        $ret = $this->format_ret(1,$jpg);
        echo json_encode($ret);
    }//download  end
	 /** 图片等比例压缩
     ...
     */
    function resizeImage($imgSrc,$maxwidth ,$maxheight ,$filetype )
    {
        //初始化图象
        if ($filetype == "jpg") {
...
//        $route = "./uploads/microblog/".$name;//压缩图片的位置
        $route = "/alidata/www/weixiaotong2016/uploads/microblog/".$name;//压缩图片的位置
        ...
        if(($maxwidth && $pic_width > $maxwidth) || ($maxheight && $pic_height > $maxheight)){
            ...
            imagedestroy($newim);
            //file_put_contents('./log/log.txt',"1111");

        }
        else{
            imagejpeg($im,$route);
            imagedestroy($im);
           // file_put_contents('./log/log.txt',"22222");
        }
    }
	
	    //获取考勤记录
    public function GetTeacherAttendancesList(){
        //定义数据模型
        $Attendancel_Model=M("attendances");
        //接收数据参数
        $userid=intval(I("userid"));
        $begintime=date("Y-m-d",I("begintime"));
        $endtime=date("Y-m-d",I("endtime"));
        if(!empty($userid)){
            $sql="select a.id,a.userid,a.arrivetime,a.schoolid,a.arrivedate,a.leavepicture,a.arrivevideo,a.create_time,a.attendancetype,b.name,b.photo from
                  wxt_attendances a join wxt_wxtuser b on a.userid=b.id 
                  where(a.userid='$userid' and(a.arrivedate between '$begintime' and '$endtime') and(a.attendancetype=1 or a.attendancetype=4)) order by a.create_time asc";
            $model = M();
            $lists = $model->query($sql);
            if($lists){
            $item=array();
            foreach($lists as $k=>$v){//对同一天的签到,签退考勤记录进行处理合并
                if(!isset($item[$v['arrivedate']])){
                    $item[$v['arrivedate']]=$v;
                   // $item[$k]=$v;
                }else{
                    $item[$v['arrivedate']]['leavetime']=$v['arrivetime'];
                }
            }
            $i=0;
            foreach($item as $key => $value){//关联数组转换成索引数组
                    $newArr[$i] = $value;
                    $i++;
            }
                $ret = $this->format_ret(1,$newArr);
            }else{
                $ret = $this->format_ret(0,"no data");
            }
        }else{
	 ...
	 //获取老师考勤记录
    public function GetTeaAttendancesList(){
        //定义数据模型
        $Attendancel_Model=M("teacher_attendances");
        //接收数据参数
        $userid=intval(I("userid"));
        $begintime=date("Y-m-d",I("begintime"));
        $endtime=date("Y-m-d",I("endtime"));
        if(!empty($userid)){
            $sql="select a.id,a.userid,a.arrivetime,a.schoolid,a.arrivedate,a.leavepicture,a.arrivevideo,a.create_time,a.attendancetype,b.name,b.photo from
                  wxt_teacher_attendances a join wxt_wxtuser b on a.userid=b.id 
                  where(a.userid='$userid' and(a.arrivedate between '$begintime' and '$endtime') and(a.attendancetype=1 or a.attendancetype=4)) order by a.create_time asc";
            $model = M();
            $lists = $model->query($sql);
            if($lists){
    ...
    public function resign(){
        ...
        $userid = I('userid');
        $classid = I('classid');
        $sign_date=I('sign_date'); //新增
        if(I("type")){ //考勤来源类型
            $type = I("type");
        }else{
            $type = 2;
        }
        if(!empty($schoolid) && !empty($userid) && !empty($classid) && !empty($sign_date)){
            $userid_arr=explode(",",$userid);
            //去掉数组中的空格位置
            $userid_arr = array_filter($userid_arr);
            //将拆分后的id用foreach和settype函数转换为单个int值
            foreach ($userid_arr as &$user_id) {
                $user_id_info=$user_id;
                $user_id=settype($user_id_info,"integer");
                $arrivedate=date("Y-m-d",$sign_date);//考勤当天日期

                //查找学生对应卡号
                // echo $user_id_info;
                $where['personId'] =  $user_id_info;
                $where['schoolid'] = $schoolid;
                $card_info = M("student_card")->field("cardNo")->where($where)->find();
                if(!empty($card_info)){
                    $data_re['cardno'] = $card_info['cardno'];//学生对应的卡号
                }else{
                    $data_re['cardno']="";
                }
                //将接收人id存入表中
                $data_re['schoolid']=$schoolid; //学校ID
                $data_re['classid']=$classid; //学校ID
                $data_re['userid']=$user_id_info;//学生ID
                $data_re['attendancetype']=1;   //考勤类型为上午上学
                $data_re['arrivetime']=time(); //考勤时间 arrivetime
                $data_re['arrivedate']=$arrivedate;  //考勤的当天日期 arrivedate
                $data_re['create_time']=time();  //创建这条记录的时间
                $data_re['source']=$type;  //渠道来源：1 app 2、微信
                $receive_ret=M('attendances')->add($data_re);
            }
            if($receive_ret){
                $ret = $this->format_ret(1,"成功");
            }else{
                $ret = $this->format_ret(0,"失败");
            }
        }else{
            $ret = $this->format_ret(0,"参数不足");
        }
        echo json_encode($ret);
        exit;
    }
    ...
    public function LeaveTime(){
        ...
        if(!empty($schoolid) && !empty($userid) && !empty($classid) && !empty($sign_date)){
            $attendance_model=M('attendances');
            $userid_arr=explode(",",$userid);
            //去掉数组中的空格位置
            $userid_arr = array_filter($userid_arr);
            //将拆分后的id用foreach和settype函数转换为单个int值
            for ($i=0; $i < count($userid_arr); $i++) {
                $where['personId'] =  $userid_arr[$i];
                $where['schoolid'] = $schoolid;
                $card_info = M("student_card")->field("cardNo")->where($where)->find();
                if(!empty($card_info)){
                    $data_add['cardno'] = $card_info['cardno'];//学生对应的卡号
                }else{
                    $data_add['cardno']="";
                }
                $arrivedate=date("Y-m-d",$sign_date);//签退当天当天日期
                $data_add['create_time']=time();
                $data_add['arrivetime']=time();
                $data_add["schoolid"]=$schoolid;//学校ID
                $data_add["classid"]=$classid;//班级ID
                $data_add["userid"]=$userid_arr[$i];//学生ID
                $data_add["arrivedate"]=$arrivedate;//签退当天当天日期
                $data_add["attendancetype"]=4; //类型
                $data_add['source']=$type;  //渠道来源：1 app 2、微信
                $attendance_add=$attendance_model->add($data_add);
                if($attendance_add){
                    $ret = $this->format_ret(1,"成功");
                }else{
	 ...
	 //获取宝宝相册列表
    public function GetbaobaoMicroblog(){
        $schoolid = I('schoolid');
        $classid = I('classid');
        $userid = I('userid'); //学生ID
        $type = I('type');
        $microblig = M('microblog_main');   //博客主表
        $microblig_pic = M('microblog_picture_url');//博客图片表
        $microblig_like = M('likes');//博客点赞表
        ...
        if($schoolid!=''&&$classid!=''&&$userid!=''){
                ...
                $userwhere["wxt_microblog_main.classid"]=$classid;
                $userwhere["wxt_microblog_main.status"]=1;
                $userwhere["wxt_microblog_main.userid"]=$userid;
                $userwhere["wxt_microblog_main.type"]=$type;
                $user_return = $microblig->join("wxt_wxtuser ON wxt_microblog_main.userid = wxt_wxtuser.id")
                    ->order("write_time DESC")
                    ->where($userwhere)
                    ->field('mid,type,title,wxt_microblog_main.schoolid,wxt_microblog_main.classid,userid,name,content,write_time,photo')
                    ->select();
            ...
            for($i=0;$i<count($user_return);$i++){
                ...
                for ($j=0; $j < count($pic_return); $j++) {
                    # code...
                    //$imagesize = getimagesize("./uploads/microblog/".$pic_return[$j]["pictureurl"]);
                    $imagesize = getimagesize("./uploads/microbloghub/".$pic_return[$j]["pictureurl"]);
                    $user_return[$i]['pic'][$j]["picturewidth"]=$imagesize["0"];
                    $user_return[$i]['pic'][$j]["pictureheight"]=$imagesize["1"];
                }
                $where_like["wxt_likes.refid"]=$microblogid;
