(function ($) {
    ...
    jQuery.Album = function (config) {
        this.setting = {
            ...
            Carousel: new jQuery.Carousel({
                ...
                pages: {prev: '#album-carousel-btn-prev', next: '#album-carousel-btn-next'},
                direction: 'H'
            }),
            curIndex: 0,
            maxWidth: 1000,
    ...
    jQuery.Album.prototype = {
        init: function () {
            ...
            var Album = this, currentItem = Album.currentItem, image = Album.getImage(), title = Album.getTitle(), overlay = Album.getOverlay(), btnDownload = Album.getBtnDownload(), src = currentItem.find("a:first").attr("href"), alt = currentItem.find("img:first").attr("alt"), percent = 0, preloader = new Image();
            overlay.removeClass(CLS_HIDE);
            this.isloading = true;
            preloader.src = src;
            $(preloader).load(function (evt) {
                var width = preloader.width, height = preloader.height, maxWidth = Album.getMaxWidth(), maxHeight = Album.getMaxHeight();
            ...
            var selectedItem = this.selectedItem, currentItem = null, items = this.getItems(), curIndex = this.getCurrentIndex();
            if (selectedItem) {
            ...
            var Album = this, root = Album.getRoot(), navBlock = Album.getNavBlock(), prevBlock = Album.getPrevBlock(), nextBlock = Album.getNextBlock(), btnPrev = Album.getBtnPrev(), btnNext = Album.getBtnNext(), items = Album.getItems();
            root.hover(function () {
