...
class SchoolInfosManageController extends TeacherbaseController {
    ...
    function _initialize() {
        ...
        $this->schoolinfo_model=D("Common/schoolinfos");
    }
...
//        $where= join(" and ", $where_ands);
    ...
    $where["schoolid"] = $_SESSION["schoolid"] ;
        $count=$this->schoolinfo_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->schoolinfo_model
        ...
        foreach ($posts as &$info) {
            ...
            ->alias("school")
            ->join("wxt_schoolinfos info")
            ->where(array("school.schoolid"=>$info['schoolid']))
            ->field('school_name')
            ->find();

            $info["schoolname"]=$schoolname["school_name"];
            unset($info);
        }
...
//------------------增加校园招聘----------------
  function add(){
      ...
      $schoolid = intval(I("get.school"));
        $this->_getTree();
        $info=$this->schoolinfo_model->where("schoolid=$schoolid")->find();
...
//------------------更新校园招聘开始----------------
    public function edit(){
        ...
        $info = $this->schoolinfo_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $info['smeta']=json_encode($_POST['smeta']);
            $info['post_content']=htmlspecialchars_decode($info['post_content']);
            $result=$this->schoolinfo_model->where($where)->save($info);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
...
//------------------删除信息------------------------
    function delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst = $this->schoolinfo_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
