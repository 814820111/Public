...
class SingleMenuController extends WeixinbaseController
{

//第一步：用户同意授权，获取code
function accept(){
    if(I("appid") and I("action") and I("type") and I("schoolid")){
        $id = I("appid");//公众号ID
        ...
        if($result){
            ...
            $REDIRECT_URI = "$http/index.php/weixin/SingleMenu/getCode?action=".$action."&type=".$type;
...
//    header("location:".$http."/index.php/weixin/Menusession/index?schoolid=".$schoolid."&action=".$action."&type=".$type);
    header("location:".$http."/index.php/weixin/SingleMenusession/index?action=".$action."&type=".$type);
