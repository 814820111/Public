...
class TeacherController extends AppBaseController {
	...
	public function index() {
	    ...
	    //学生考勤统计
    public function  stuAttendancesCount()
    {
        $userid = I('userid'); //老师ID
        $schoolid = I('schoolid'); //学校ID
        if(I('sign_date')){
            $time = date("Y-m-d",I('sign_date')); //传过来的时间
            $sign_date = I('sign_date'); //传过来的时间
        }else{
...
//        $time = date("Y-m-d",time());
        ...
        foreach ($class as $k=>&$value) {
            $classid = $value['classid'];//班级ID
            $classname = $value['classname'];//班级名称

            //学生总人数
            $stu_num = M("class_relationship")->where(array("schoolid"=>$schoolid,"classid"=>$classid))->select();
            //学生签到人数
            $sign_num = M("attendances")->where(array("schoolid"=>$schoolid,"classid"=>$classid,"arrivedate"=> $time,"attendancetype"=>1))->count();
            //学生签退人数
//            $signout_num = M("attendances")->where(array("schoolid"=>$schoolid,"classid"=>$classid,"arrivedate"=> $time,"attendancetype"=>4))->count();
            $res[$k]['classid'] = $classid;
            $res[$k]['classname'] = $classname;
            $res[$k]['stu_num'] = count($stu_num);
            $res[$k]['sign_num'] = $sign_num;

//            $res[$k]['signout_num'] = $signout_num;

            //获得学生请假记录
            $leave_info = M("leave")->where("'$sign_date' between begintime and endtime")->select();
            foreach($stu_num as &$stuvalue){ //拼接请假记录
                foreach($leave_info as &$leavevalue){
                    if($stuvalue['userid'] == $leavevalue['studentid'] && $leavevalue['status']==1 ){ //1表示已同意的请假记录
                        $leave_num =  $leave_num+1;
                    }
                }
            }
            $res[$k]['leave_num'] = $leave_num; //学生请假人数
            $res[$k]['wei_num'] = count($stu_num)-$sign_num-$leave_num;
        }
    ...
    public function count(){
        ...
        $time = date("Y-m-d",time());//当天
        //学生列表
        $stu_num = M("class_relationship")->where(array("schoolid"=>$schoolid,"classid"=>$classid))->select();
        //学生签到列表
        $sign_num = M("attendances")->where(array("schoolid"=>$schoolid,"classid"=>$classid,"arrivedate"=> $time,"attendancetype"=>1))->select();
        //学生签退列表
//        $signout_num = M("attendances")->where(array("schoolid"=>$schoolid,"classid"=>$classid,"arrivedate"=> $time,"attendancetype"=>4))->select();
        //获取学生总数 和家长总数
        $parent_num = 0;
        for ($i = 0;$i<count($stu_num);$i++){
            ...
            if ($result){
                $parent_num =count($result)+$parent_num;
            }
        }
        //获得学生请假记录
        $sign_date = strtotime($time);
        $leave_info = M("leave")->where("'$sign_date' between begintime and endtime")->select();
//        dump($leave_info);
        $leave_num = 0;
        foreach($stu_num as &$stuvalue){ //拼接请假记录
        ...
        //获得老师总人数
        $teacher_num = M('teacher_info') ->where("schoolid = $schoolid")->count();
        //老师签到人数
        $ter_sign_num = M("teacher_attendances")->where(array("schoolid"=>$schoolid,"arrivedate"=> $time,"attendancetype"=>1))->count();
        //老师签退人数
//        $ter_signout_num = M("teacher_attendances")->where(array("schoolid"=>$schoolid,"arrivedate"=> $time,"attendancetype"=>4))->count();
        $data['teacher_num'] = $teacher_num;//老师总人数
        $data['ter_sign_num'] = $ter_sign_num;//老师签到人数
        $data['ter_wei_num'] = $teacher_num-$ter_sign_num;//老师签到人数
//        $data['ter_signout_num'] =  $ter_signout_num;//老师签退

        $stu_num = count($stu_num);
        $wei_num = $stu_num- count($sign_num)-$leave_num;
        $data['stu_num'] = $stu_num;//学生总数
        $data['parent_num'] = $parent_num; //家长总数
        $data['sign_num'] = count($sign_num);//学生签到人数
//        $data['signout_num'] = count($signout_num);//学生签退人数
        $data['leave_num'] = $leave_num;//学生请假人数
        $data['leave_num'] = $leave_num;//学生请假人数
        $data['wei_num'] = $wei_num;//未到人数
        if ($stu_num){
    ...
    //老师考勤统计
    public function  terAttendancesCount()
    {
        $schoolid = I("schoolid");
        if(I('sign_date')){
            $time = date("Y-m-d",I('sign_date')); //传过来的时间
        ...
        $teacher_num = M('teacher_info') ->where("schoolid = $schoolid")->count();
        //老师签到人数
        $ter_sign = M("teacher_attendances as a")->
        join("wxt_wxtuser as b on a.userid=b.id")->
        where(array("a.schoolid"=>$schoolid,"a.arrivedate"=> $time,"a.attendancetype"=>1))->
        field("b.id,b.photo,b.name,b.phone")->
        select();
//        print_r($ter_sign);
        $new_arr=array();
        foreach($ter_sign as $key => $value){
            $new_arr[$key] = $value['id'];
        }
        $new_arr = implode(",",$new_arr);
        $where['a.schoolid'] = $schoolid;
        $where['a.teacherid'] =  array('not in',$new_arr);
        $wei_info = M('teacher_info as a') ->
                    join("wxt_wxtuser as b on a.teacherid=b.id")->
                    where($where)->
                    field("b.id,b.photo,b.name,b.phone")->
                    select();
//        //老师签退人数
//        $ter_signout = M("teacher_attendances as a")->
//        join("wxt_wxtuser as b on a.userid=b.id")->
//        where(array("a.schoolid"=>$schoolid,"a.arrivedate"=> $time,"a.attendancetype"=>4))->
//        field("b.id,b.photo,b.name,b.phone")->
        //select();
        $data['teacher_num'] = $teacher_num;//老师总人数
        $data['ter_sign_num'] = count($ter_sign);//老师签到人数
        $data['ter_sign_info'] = $ter_sign;//老师签到详情
       // $data['ter_signout_num'] =  count($ter_signout);//老师签退
        //$data['ter_signout_info'] =  $ter_signout;//老师签退
         $data['ter_wei_num'] =  count($wei_info);//老师未到人数
         $data['wei_info'] =  $wei_info;//老师未到人数详情
...
//            $ret = $this->format_ret(0,$data);
    ...
    exit;
        ...
        //前台登录验证
    public function teacher_login_pc_Error(){
        //获取传入的参数phone,password；赋值给$mobile,$pass
        $mobile = I('phone');
        $password = I('password');
        $verify=I('verify');
...
//                $ret = $this->format_ret(1,$homeworkid);
                $ret = array("id"=>$homeworkid,"status"=>"成功");
...
//                $ret = $this->format_ret(0,"add error");
	...
	//老师获取作业信息列表 详情
    public function gethomeworklist_detasils(){
   ...
   //发布班级活动
    public function addactivity(){
        ...
        if($teacherid &&(!empty($title)) && (!empty($content))){
            ...
            if($homeworkid){
//                $ret = $this->format_ret(1,$homeworkid);
                $ret = array("id"=>$homeworkid,"status"=>"失败");
