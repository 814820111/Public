...
var mobile = {
    ...
    pageContent: function () {
        ...
        $.ajax({
            ...
            url: root + '/weixin/TchGrowtharchives/studentPageList',
                    ...
                    if (arr[i]['text_count']) {
                        ...
                        for (var j = 0; j < res.length; j++) {
                            if (res[j]['type'] == 0) {
                                ...
                                html += '<textarea id="texts" style="width: ' + w + 'px; height: ' + h + 'px;left: ' + l + 'px;top: ' + t + 'px;border:none;background-color:transparent;resize:none;color:#8f5731;font-size:16px;font-family:微软雅黑,Helvetica" draggable="false" placeholder="请从输入文字...">';
                                if (res[j]['text']) {
                                    html += res[j]['text'];
                                }
                                html += '</textarea></div>'
    ...
    uploadServer: function (file, page){
        $.ajax({
            ...
            url: root + "/Apps/QiNiu/GrowtharchivesUpload",
            type: "POST",
            data: file,
    ...
    show: function () {
        $('#caseViolette').css('display', 'block');
        $('.edit_bg').css('display', 'block');
        $('.edit_bg').animate({opacity: .5}, 0);
    },
    hide: function () {
        $('#caseViolette').css('display', 'none');
        $('.edit_bg').css('display', 'none');
    ...
    saveAjax: function (img, text, isOk) {
        //console.log(isOk);
        $.ajax({
            url: root + "/weixin/TchGrowtharchives/save",
            type: "POST",
            ...
            success: function (res) {
                if (res.status == 200) {
                    ...
                    location.href = root+"/weixin/TchGrowtharchives/studentPage?cid="+cid+"&sid="+sid+"&gid="+gid+"&aid="+aid;
                } else {
                    alert("保存失败");
                }
            },
    ...
    selectClassPhoto: function (tabsSwiper, id, img, page) {
        ...
        tabsSwiper.slideTo((parseInt(page) - 1), 1000, false);//切换到第一个slide，速度为1秒
    },
