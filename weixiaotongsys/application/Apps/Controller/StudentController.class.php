        ...
        if($studentid){
            ...
            foreach ($microblog_list as &$like_com) {
                ...
                for ($j=0; $j < count($pic); $j++) { 
					...
					$imagesize = getimagesize("/alidata/www/weixiaotong2016/uploads/microbloghub/".$pic[$j]["pictureurl"]);
...
//家长端-获取班级活动信息
	...
	//家长端-获取班级活动信息详情
    public function getactivitylist_details(){
        //传入接收人id
        $receiverid=Intval(I('receiverid'));
        ...
        if($userid){
         ...
         $comment_model = M('comments');//评论表
            foreach ($list as &$entrust ) {
                ...
                $teacherid=$entrust["teacherid"];//修改的代码
