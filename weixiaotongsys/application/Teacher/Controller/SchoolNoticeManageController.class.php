...
class SchoolNoticeManageController extends TeacherbaseController {
...
//        $where= join(" and ", $where_ands);
    ...
    $where["schoolid"] = $_SESSION["schoolid"] ;
        $count=$this->schoolnotice_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->schoolnotice_model
        ...
        foreach ($posts as &$notice) {
            ...
            ->alias("school")
            ->join("wxt_schoolnotice notice")
            ->where(array("school.schoolid"=>$notice['schoolid']))
            ->field('school_name')
            ->find();

            $notice["schoolname"]=$schoolname["school_name"];
            unset($notice);
        }
...
//------------------增加校园招聘----------------
  function add(){
      ...
      $schoolid = intval(I("get.school"));
        $this->_getTree();
        $notice=$this->schoolnotice_model->where("schoolid=$schoolid")->find();
    ...
    function add_post(){
        if (IS_POST) {
           ...
           //$_POST['post']['post_author']=get_current_admin_id();
            $notice=I("post.post");
            $notice['schoolid']=$_POST['school'];
            // $schoolname=$this->school_model->where("$school_id=schoolid")->getField('school_name');

            $notice['smeta']=json_encode($_POST['smeta']);
            $notice['post_content']=htmlspecialchars_decode($notice['post_content']);
            $result=$this->schoolnotice_model->add($notice);
            if ($result) {
...
//------------------更新校园招聘开始----------------
    public function edit(){
        ...
        $notice = $this->schoolnotice_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $notice['smeta']=json_encode($_POST['smeta']);
            $notice['post_content']=htmlspecialchars_decode($notice['post_content']);
            $result=$this->schoolnotice_model->where($where)->save($notice);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
...
//------------------删除信息------------------------
    function delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst = $this->schoolnotice_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
