...
class JobManageController extends TeacherbaseController {
...
//        $where= join(" and ", $where_ands);
        ...
        $count=$this->job_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->job_model
        ...
        foreach ($posts as &$job) {
            ...
            ->alias("school")
            ->join("wxt_jobs job")
            ->where(array("school.schoolid"=>$job['schoolid']))
            ->field('school_name')
            ->find();

            $job["schoolname"]=$schoolname["school_name"];
            unset($job);
        }
...
//        $school_name = $this->school_model->order(array("schoolid"=>"asc"))->select();
        ...
        $this->_getTree();
        $job=$this->job_model->where("schoolid=$schoolid")->find();
    ...
    function add_post(){
        if (IS_POST) {
           ...
           //$_POST['post']['post_author']=get_current_admin_id();
            ...
            $job['smeta']=json_encode($_POST['smeta']);
            $job['post_content']=htmlspecialchars_decode($job['post_content']);
            $result=$this->job_model->add($job);
            if ($result) {
...
//------------------更新校园招聘开始----------------
    public function edit(){
        ...
        $job = $this->job_model->where(array("id"=>$id))->find();
    ...
    public function edit_post(){
        if (IS_POST) {
            ...
            $job['smeta']=json_encode($_POST['smeta']);
            $job['post_content']=htmlspecialchars_decode($job['post_content']);
            $result=$this->job_model->where($where)->save($job);
            if ($result!==false) {
                $this->success("保存成功！");
            } else {
                $this->error("保存失败！");
