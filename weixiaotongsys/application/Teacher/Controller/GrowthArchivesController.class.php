...
class GrowthArchivesController extends TeacherbaseController
{
    ...
    public function index()
    {
        ...
        //获得档案信息
        $semester = M('semester')->field('semester')->select();
        //print_r($semester);
        ...
        if ($semester) {
            $this->assign("semester1", $semester);
...
//        print_r($gradelist);
    ...
    public function add()
    {
        ...
        //获得该校关联学期
        $semester = M('semester')->field('id,semester')->select();
        $this->assign('semester', $semester);//学期
        $this->assign('schoolid', $schoolid);//学校ID
...
//       }
    ...
    public function addBook()
    {
        ...
        if (I('pageid')) {//书页ID
            ...
            $student = M('archives_student')
                ->where("gradeid=$gradeid and archivesid=$archivesid")
                ->field('studentid')
                ->select();
            foreach ($student as $ks => $vs) {
                foreach ($page as $k => $value) {
                    $pageid = $value;
                    $datas['student_id'] = $vs['studentid'];
                    $datas['grade_id'] = $gradeid;
                    $datas['archives_id'] = $archivesid;
                    $datas['page_id'] = $pageid;
                    $is = M('growth_student_temple')->where($datas)->find();
                    if (!$is) {
                        $id = M('growth_student_temple')->add($datas);
                        if ($id) {
                            //根据页获取当前页的详细信息
                            $imgTexts = M()->query("select tit.id from wxt_temple_imgs_texts as tit where tit.temple_id= (select ap.temple_id from wxt_archives_page as ap where ap.id=$pageid)");
            ...
            foreach ($page as $k => $value) {
                $pageid = $value;
                $array = M('archives_page')->where("id=$pageid")->field('writen')->find();
                //dump($array);
                $data['pageid'] = $pageid;
                $data['gradeid'] = $gradeid;
                $data['archivesid'] = $archivesid;
                ...
                $data['create_time'] = time();
                $data['writer'] = $array['writen'];
                //向档案书本表中插入数据
                $result = M("archives_book")->add($data);
            }
...
//              unset($grade_item);
            ...
            $where['b.gradeid'] = array("in",$teacher_grade);
...
//            ->where("a.schoolid=$schoolid and a.gradeid=$gradeid and a.state=3")->select();
//
//        if($result){
//            echo json_encode(array('data'=>$result,'message'=>'数据获取成功'));
//        }else{
//            //如果没有根据年级ID 和学校ID 拉取    state 为2   系统模板
//            $query = M('evaluation as a')->join('wxt_evaluation_content as b on a.id=b.evaluationid')
//                ->where("a.state=2 and a.schoolid= $schoolid and a.gradeid=$gradeid")->select();
//            if($query){
//                echo json_encode(array('data'=>$query,'message'=>'数据获取成功'));
//            }else{
//                //拉取取默认 state为1的 学校ID为0 的系统默认模板
//                $arr = M('evaluation as a')->join('wxt_evaluation_content as b on a.id=b.evaluationid')
//                    ->where("a.state=1 and a.schoolid=0")->select();
//                echo json_encode(array('data'=>$arr,'message'=>'数据获取成功'));
//            }
//
//        }
//
...
//           {
//               $id = $value;//考评项ID
//               $result = M('evaluation')->where("id=$id and $state=2 ")->find(); //第一次取默认模板
//               $data['name']=$result['name'];
//               $data['schoolid']= $schoolid;
//               $data['gradeid']=$gradeid;
//               $data['type']= $type;
...
//            {
//                $id = $value;//考评项ID
//                $result = M('evaluation')->where("id=$id and $state=3 ")->find(); //第一次取默认模板
//                $data['name']=$result['name'];
//                $data['schoolid']= $schoolid;
//                $data['gradeid']=$gradeid;
//                $data['type']= $type;
...
//    }
 ...
 // var_dump($data);
    ...
    public function show()
    {
        ...
        $aa = M('archives_student')->where("schoolid=$schoolid")->select();
        //var_dump($aa);
        if($this->level!=1  && $this->level!=2)
        {
        ...
        $list = M('archives_student')
            ...
            ->join("wxt_school_grade g ON g.gradeid = a.gradeid")
            ->join($join_duty)
            ->field("a.id,a.archivesid,a.studentid,s.stu_name,g.name as grade_name,c.classname")
            ->select();
        //var_dump($list);
        foreach ($list as $key => &$value) {
            ...
            $where_count['studentid'] = $studentid;
            $where_count['archivesid'] = $archivesid;
            $where_count['schedule'] = 1;
            // $where_count['type'] = 1;


            //查出该档案的总页数
            $archives_detail = M('archives_book')->where("archivesid = $archivesid")->field("count(1) as sum")->group('writer')->select();
            ...
            //查出该学生下完成进度
            $detail = M('archives_student_page')->where($where_count)->group('writen')->field("count(1) as sum")->select();
            ...
            $value['tea_sum'] = $detail[0]['sum'] . '/' . $archives_detail[0]['sum'];
            $value['par_sum'] = $detail[1]['sum'] . '/' . $archives_detail[1]['sum'];
...
//                }
                ...
                //获取已完成页数
                $asp = M('archives_student_page')->where("archivesid=$archivesid and studentid={$v['studentid']} and schedule=1")->select();
...
//        }
        ...
        $count = M('archives_student_page')->where("archivesid and $aid and studentid = $sid and schedule = 1")->count();
