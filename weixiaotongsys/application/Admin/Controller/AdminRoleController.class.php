                ...
                if ($this->role_model->add()!==false) {
                    ...
                    if($role==1)
                    {
                        ...
                        if($categoryid==1)
                        {
                            if($city && $Province)
                            {
                                $where_school['province'] =$Province;
                                $where_school['city'] = $city;
                            }
                            if($Province)
                            {
                                $where_school['province'] = $Province;
                            ...
                            //如果是内部管理员
                            $school = M("school")->where($where_school)->field('province,city,area,schoolid')->select();
        ...
        if (IS_POST) {
            ...
            if ($data) {
                if ($this->role_model->save($data)!==false) {
                    $this->success("修改成功！", U('AdminRole/index'));
                } else {
                    $this->error("修改失败！");
                }
            } else {
