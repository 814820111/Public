...
class NoticeController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->photo_model=D("Common/notice_photo");
    }
...
private  function _lists($status=1){
        ...
        $count=$this->notice_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->notice_model
        ->field('id,title,userid,content,create_time,type,status')
        ->where($where)
        ...
        foreach ($posts as &$photo) {
             $smeta=$this->photo_model
            ->where(array("noticeid"=>$photo['id']))
            ->field('photo')
            ->find();

            $photo["smeta"]=$smeta["photo"];
            unset($photo);
        }
...
//------------------删除信息------------------------
    function delete(){
        ...
        //var_dump($id);
        if ($id) {
            $rst = $this->notice_model->where("id=$id")->delete();
            if ($rst) {
                $this->success("删除成功！");
            } else {
                $this->error("删除失败！");
