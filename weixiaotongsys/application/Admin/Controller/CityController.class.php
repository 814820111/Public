...
class CityController extends AdminbaseController{
	...
	function _initialize() {
		parent::_initialize();
		$this->terms_model = D("City/City");
	}
	function index(){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
		// $this->terms_model = M("City");
		$tree = new \Tree();
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($result as $r) {
			$r['str_manage'] = '<a href="' . U("City/add", array("parent" => $r['term_id'])) . '">添加下级城市</a> | <a href="' . U("City/edit", array("id" => $r['term_id'])) . '">编辑</a> | <a class="js-ajax-delete" href="' . U("City/delete", array("id" => $r['term_id'])) . '">删除</a> ';
			// $r['url'] = $url;
