...
class SchoolPlanController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->plan_model=D("Common/school_plan");
    }
...
private  function _lists($status=1){
        ...
        $count=$this->plan_model
        ->where($where)
        ->count();
            
        $page = $this->page($count, 20);
            
        $posts=$this->plan_model
        ->field('id,schoolid,classid,type,title,monday,school_phone,create_time,school_status')
        ->where($where)
