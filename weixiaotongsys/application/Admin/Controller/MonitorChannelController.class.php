...
class MonitorChannelController extends AdminbaseController
{
    ...
    public function index()
    {
        ...
        $citys = I('citys');
        if($area){
            $this->assign("new_message_type",$area);
        }
        if($province){
        ...
        if($school){
            $count= M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->where("s.schoolid = '$school' ")->count();
            $page = $this->page($count, 15);
            $info = M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->limit($page->firstRow . ',' . $page->listRows)->order('c.id desc')->field("c.id,s.school_name,c.nname,c.deviceSerial, c.channelName")->where("s.schoolid = '$school' ")->select();
            $this->assign("schoolid",$school);
        }elseif($area){
            $count= M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->where("s.school_name like '$keyword%' and s.area = '$area' ")->count();
            $page = $this->page($count, 15);
            $info = M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->limit($page->firstRow . ',' . $page->listRows)->order('c.id desc')->field("c.id,s.school_name,c.nname,c.deviceSerial, c.channelName")->where("s.school_name like '$keyword%' and s.area = '$area' ")->select();
        }else{
            $count= M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->where("s.school_name like '$keyword%'")->count();
            $page = $this->page($count, 15);
            $info = M('monitor_video')->alias("c")->join("wxt_school as s on c.school_id = s.schoolid")->limit($page->firstRow . ',' . $page->listRows)->order('c.id desc')->field("c.id,s.school_name,c.nname,c.deviceSerial, c.channelName")->where("s.school_name like '$keyword%'")->select();
        }
    ...
    public function edit()
    {
        ...
        $id = I('id');
        //获取名称
        $monitorChannel = M('monitor_video')->where("id = $id")->field("nname")->find();
    ...
    {
        $id = I('id');
        $res = M('monitor_video')->where("id = $id")->delete();
        if ($res) {
...
//获取萤石的token
    ...
    //获取设备摄像头方法
    function getDeviceCamaroList($token, $deviceserial)
    {
        $post_data = "&accessToken=" . $token."&deviceSerial=$deviceserial";
        $this_header = array("application/x-www-form-urlencoded; charset=gbk");
    ...
    public function deleteAll(){
        ...
        foreach ($ids as $v){
            $res = M('monitor_video')->where("id = $v")->delete();
        }
