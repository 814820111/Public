      ...
      if($schoolid)
      {
       ...
       if($message_type)
       {
        ...
        if (!empty($get_keywordtype)&&!empty($get_keyword)&&$get_keywordtype=='name'){
            ...
            $map['t.name'] = array("like","%".$get_keyword."%");
...
//        }
        foreach ($teachers as &$teacherinfo) {
            ...
            ->where(array("t.teacher_id"=>$teacherinfo['info_id']))
   ...
   public function teacher_post(){
        ...
        if(!$info){
           ...
           // var_dump($highestColumn);
            ...
            {
                //循环数据检测B=>教师姓名，C=>教师角色，D=>电话号码，E=>校园短号，F=>IC卡号，G=>班级，H=>科目，I=>班主任班级
                //如果题号或者题干不为空
...
//                    'huanxinid' => $huanxinid,
            ...
            if ($userid){
                ...
                if($teacher_info_find_result){
                    M("teacher_info")->where($teacher_info_find)->save(array("name"=>$username));
                    update_card(1,$userid,$schoolid);
                    $result = 1;
                    return $result;
                }else{
   ...
   $where['t.schoolid'] = $schoolid;
    $teachers = M('teacher_info')
		...
		foreach ($teachers as &$teacherchange) {
			...
			->alias("d")
			->join("wxt_duty_teacher t ON d.id=t.duty_id")
            ->where(array("t.teacher_id"=>$teacherchange['id'],"t.schoolid"=>$schoolid))
            ->field('name')
            ->select();
            $dutyStr = "";
            foreach ($dutylist as &$duty){
        ...
        if ($teacherid) {
           ...
           $aa = $rst['birthday'];
             ...
             $birthday = date('Y-m-d',$aa);
              ...
              $card = M('student_card')->where("cardType = 1 and handletype = 1 and personId = $info_id")->field("cardNo")->find();
	...
	function edit_post(){
		if (IS_POST) {
            ...
            $edit_phone = edit_phone(1,$_POST['phone'],$info_id,$schoolid);
            ...
            if($email || $description || $data['name'])
            {
              ...
              $teacher_info  = M("teacher_info")->where("teacherid = $teacherid and schoolid = $schoolid")->save($info);
            ...
            foreach ($department as $val) {
                ...
                if($_POST['name'] || $_POST['smeta']['thumb'])
                {
                     ...
                     $card_save = M("student_card")->where("personId = $info_id and handletype = 1 and cardType = 1")->save($cardinfo);

                  }
            $result = M('department_teacher')->addAll($alldata); 

               $where_card['cardno']=$oldCardNo;
               $where_card['personId']=$info_id;
    ...
    public function departmentedit()
    {
      ...
      public function  departmentpost()
      {
        ...
        $teacherid = I("teacherid");
          ...
          $info_id = M("teacher_info")->where(array("teacherid"=>$teacherid,"schoolid"=>$schoolid))->getField("id");
	...
	function add_post(){
		if (IS_POST) {
			...
			$sex=Intval(I("sex"));
             $str=$_POST['smeta']['thumb'];
             $arr=explode("/", $str);
             $avatar=!$arr[count($arr)-1]?"weixiaotong.png":$arr[count($arr)-1];
          ...
          if($teacher_user)
          {
            ...
            if(M("teacher_info")->where(array("teacherid"=>$teacherid,"schoolid"=>$schoolid))->find())
            {
                $this->error("该老师已经存在该学校,请勿重复添加");
                exit();
            }
            ...
            //根据传过来职责id去查是否是学校管理员
                    $duty_name = M("school_duty")->where(array("id"=>$_POST['du'],"schoolid"=>$schoolid))->getField("name");
           if($duty_name=="学校管理员")
           {
               add_admin_class($teacherid,$schoolid);
           }
