...
class SchoolAppRoleController extends AdminbaseController {
...
//            dump($Province);
   ...
   if($schoolid)
   {
    	...
    	if (IS_POST) {
    		...
    		if ($data) {
    			if (M('school_duty')->save($data)!==false) {
    				$this->success("修改成功！", U('SchoolAppRole/index'));
    			} else {
    				$this->error("修改失败！");
    			}
    		} else {
    ...
    public function authorize() {
        ...
        $priv_data=$this->school_access_app->where("role_id=$roleid and appcatid = 2")->field("status,rule_name")->select();//获取权限表数据
 ...
 $html = '';
        ...
        foreach ($result as $key => $value) {
         ...
         if($required)
         {
            ...
            $input = "<input type =hidden name='status[]' value=".$value['id'].">";
        ...
        if($checked)
        {
         ...
         $html.="<input  type='checkbox'  ".$res." style='vertical-align: -1px;' checked name='menuid[]' value='".$value['id']."'  onclick='javascript:checknode(this);'>".$value['name']."".$input."</td>";
        }else{
         $html.="<input  type='checkbox' ".$res." style='vertical-align: -1px;' name='menuid[]' value='".$value['id']."'  onclick='javascript:checknode(this);'>".$value['name']."".$input."</td>";
        }
         ...
         foreach($menuid as $key => $value)
          {
    		...
    		if (is_array($menuid) && count($menuid)>0) {
    			...
    			$this->school_access_app->where(array("role_id"=>$roleid,'type'=>'app_url',"appcatid"=>"2"))->delete();
              ...
              // var_dump($status);
    					...
    					$action=$menu['action'];
    					$name=strtolower("$app/$model/$action");
    					$this->school_access_app->add(array("role_id"=>$roleid,"rule_name"=>$name,'schoolid'=>$schoolid,'type'=>'app_url','appcatid'=>2,"status"=>$status));
    				}
    			}

    			$this->success("授权成功！", U("SchoolAppRole/index",array("province"=>$province,"city"=>$citys,"message_type"=>$new_message_type,'schoolid'=>$schoolid)));
    		}else{
    			//当没有数据时，清除当前角色授权
    			$this->school_access_app->where(array("role_id" => $roleid,'type'=>'app_url',"appcatid"=>"2"))->delete();
