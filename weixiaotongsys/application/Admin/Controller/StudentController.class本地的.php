...
class StudentController extends AdminbaseController {
    ...
    function _initialize() {
        ...
        $this->only_code = strtolower(MODULE_NAME).'_'.strtolower(CONTROLLER_NAME)."_".strtolower(ACTION_NAME);
    ...
    public function parentlist() {
        ...
        $studentid = $_GET['id'];
        if ($studentid) {
            $schoolid = $this -> student_info_model->where("userid = $studentid")->getField("schoollid as schoolid");
    ...
    public function addparent_post() {
        ...
        if (IS_POST) {
            ...
            if($_POST['phone']!=13000000000)
            {
                //查找该学生的主号是否为13000000000
                $show_phone = $this->relation_stu_user_model->where(array("studentid"=>$_POST['studentid'],"type"=>1))->getField("phone");
                if($show_phone==13000000000)
                {
                    $this->relation_stu_user_model->where(array("studentid"=>$_POST['studentid'],"type"=>1))->save(array("type"=>0));
                    $d_data['type'] = 1;
                    $u_data['type'] = 1;
            ...
            if ($user_id) {
                ...
                $u_data['charging_phone'] = $_POST['phone'];
                $u_data['time'] = time();
                ...
                $d_data['charging_phone'] = $_POST['phone'];
    ...
    public function updateparent() {
        ...
        $result = $this -> relation_stu_user_model -> alias('r') -> join("" . C('DB_PREFIX') . 'wxtuser as su on r.studentid = su.id') -> join("" . C('DB_PREFIX') . 'wxtuser as pu on r.userid = pu.id') -> field("r.id,r.studentid,r.userid as parentid,r.appellation,su.name as studentname,pu.name as parentname,pu.phone as parentphone") -> where($where) -> find();
        // var_dump($result);
        $relation = $this -> appellation_model -> order(array("id" => "asc")) -> select();
        // var_dump($relation);
    ...
    public function updateparent_post() {
        ...
        if (IS_POST) {
            ...
            if ($parentid && $relationid && $appe) {
                ...
                //注释以下代码 一个学生开挂靠多个家长
//				$user_id = $this -> wxtuser_model -> where($where) -> getField('id');
//				if (!$user_id) {
                //如果新存的家长在用户表中未存在
...
//	                 $card_addc['handletime'] = time();
    ...
    public function delcard()
    {
        ...
        $delete = M('student_card')->where("cardno = $cardno and cardType = 0 and handletype = 1")->save(array("handletype"=>0));
        // var_dump($delete);
...
//            exit();
        ...
        if ($tiaojian == "name" || $tiaojian == "0" || $tiaojian == "" ) {
            ...
            //整改
            $students = $this -> wxtuser_model -> alias("bi")->distinct(true) -> join("wxt_relation_stu_user si ON bi.id=si.userid ")->join("wxt_school school ON school.schoolid = bi.schoolid")->join($join_else_info)->join($join) -> where($wheres) -> limit($page -> firstRow . ',' . $page -> listRows) -> field("si.name as names, bi.phone ,si.studentid as id,bi.schoolid") -> select();
            ...
            foreach ($students as &$student_user) {
                ...
                $uisname = $this -> student_info_model ->alias("info")-> where(array("info.userid" => $student_user['id']))->join("wxt_school school ON school.schoolid = info.schoollid") ->field("info.stu_name as name,info.create_time,info.sex,school.school_name") ->select();
    ...
    public function Lexcel() {
        ...
        if ($tiaojian == "name" || $tiaojian == "0" || $tiaojian == "") {
            ...
            $students = $this -> relation_stu_class_model -> alias("a")-> join("wxt_school_class class ON a.classid = class.id")->join("wxt_wxtuser s ON a.userid = s.id") ->join("wxt_student_info st ON s.id = st.userid") ->join($join_else_grade)->field("
				s.id,s.schoolid,s.name,s.photo,s.sex,s.create_time,st.stu_id,st.in_residence,st.nation,native_place,st.bindingkey,class.classname") -> where($where) -> order("class.listorder ASC,class.create_time desc") -> select();
...
//				$parent_phone = M("relation_stu_user")->where("studentid = $studentid and type = 1")->field("phone")->find();
                $parent_phone = M("relation_stu_user")->where("studentid = $studentid")->field("phone,name")->select();
...
//判断是否绑定微信
    ...
    public function idupload_post(){
        ...
        if($optionsRadiosinline == "option1") {
            ...
            foreach ($ExamPaper_arr as $key => $value) {
                ...
                $classname = trim($value['A']);
                $studentname = trim($value['B']);
                $card = trim($value['C']);
    ...
    function importstudent($name,$class,$parentname,$phone,$relation,$sexname,$stu_id,$card,$in_residence,$schoolid,$parent_card){
        // exit();
...
//								"photo"=>'学生.png',
                    ...
                    $phone = explode(",", $phone);
                    $parentname = explode(",", $parentname);

                    $i = 0;
                    foreach ($phone as $key=>$phone_num) {
                        $i++;
                        if($phone_num=='')
                        {
                            continue;
...
//                      }
            ...
            $res = $this -> wxtuser_model -> where("id =$userid") -> save(array('photo'=>$photo,'name'=>$name));
            if ($userid) {
...
//暂时先不删除user表            $user = $this -> wxtuser_model -> where("id=$userid") -> delete();
