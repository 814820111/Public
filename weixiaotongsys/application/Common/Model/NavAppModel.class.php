...
class NavAppModel extends CommonModel {
	//自动验证
    protected $_validate = array(
    	...
    	array('id,app,model,action,name', 'checkActionUpdate', '同样的记录已经存在2！', 1, 'callback', CommonModel:: MODEL_UPDATE   ),
     ...
     */
    public function admin_menu($cid,$parentid,$location,$with_self = false) {
        //父节点ID
        $parentid = (int) $parentid;
       
         
          
        $result = $this->where(array('appcatid'=>$cid,'type' => $parentid, 'status' => 1,'location'=>$location))->order(array("listorder" => "ASC"))->select();
     ...
     * 菜单树状结构集合 $type为菜单层次
     */
    public function menu_json($cid = 1,$type,$location) {
        $data = $this->get_tree($cid,$type,$location);
         ...
         if($cid==2)
         {
          ...
          $where['a.type'] = 'app_url';
     
        $ids = M('school_duty')
            ->alias('d')
            ->where($where)
            ->join('wxt_duty_teacher as dt on d.id=dt.duty_id')
            ->join('wxt_school_authaccess_app as a on a.role_id = d.id')
            ->field('a.rule_name')
            ->select();
          
        }else{
            $where['d.schoolid'] = $schoolid;
            $where['a.type'] = 'parent_url';
         
            $ids = M('school_duty_parent')
            ->alias('d')
            ->where($where)
            ->join('wxt_school_authaccess_app as a on a.role_id = d.id')
            ->field('a.rule_name')
    ...
    //取得树形结构的菜单
    public function get_tree($cid,$myid,$location, $parent = "", $Level = 1) {
       
        $data = $this->admin_menu($cid,$myid,$location);
        $Level++;
        if (is_array($data)) {
            foreach ($data as $a) {
                $id = $a['id'];
