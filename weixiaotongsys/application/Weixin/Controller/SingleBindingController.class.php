...
class SingleBindingController extends WeixinbaseController
{

    function accept(){
        if(I("appid") and I("schoolid")){
            $id = I("appid");//公众号ID
...
//            $_SESSION["manage_id"] = $id;
            ...
            if($result){
                ...
                $REDIRECT_URI = "$http/index.php/weixin/SingleBinding/getCode";
    ...
    function getCode(){
        ...
        $http = "http://".$_SERVER['HTTP_HOST'].__ROOT__;
        header("location:".$http."/index.php/weixin/SingleBinding/build");
...
//        }
    ...
    public function Tbuilds(){
        ...
        if(empty($result)){
            $content = "不存在这个手机号码";
            $this->Write_Weixin_TeaLog($sid,$_SESSION["APPID"],$number,$phone,2,$content);
            $ret= $this->format_ret(0,"不存在这个手机号码");
            echo json_encode($ret);
            return;
        }
        $userid = $result['id'];//获取用户ID
        $school = M("teacher_info")->where(array("teacherid"=>$userid))->field("schoolid")->select();
        foreach ($school as $value){
            $schoollist[] = $value["schoolid"];
        }
        if(in_array($_SESSION["schoolid"],array_unique($schoollist))==false){
            $content = "不能绑定其他学校的老师";
            $this->Write_Weixin_TeaLog($sid,$_SESSION["APPID"],$number,$phone,2,$content);
        ...
        $query = M('teacher_info')->where(array("teacherid"=>$userid,"bindingkey"=>$number,"schoolid"=>$_SESSION["schoolid"]))->find();
        $schoolid = $query["schoolid"];

        if(empty($query)){
            $content = "绑定码有问题，请联系学校管理员";
            $this->Write_Weixin_TeaLog($sid,$_SESSION["APPID"],$number,$phone,2,$content);
...
//        }
        ...
        if ($array){
            $content = "绑定成功";
            $this->Write_Weixin_TeaLog($sid,$_SESSION["APPID"],$number,$phone,1,$content);
