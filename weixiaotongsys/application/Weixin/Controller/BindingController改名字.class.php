...
//            if(!$schoollist){
//                $ret = format_ret(0, "该学校用户没有开通使用这个公众号的功能");
//                echo json_encode($ret);
//                die();
//            }
//        }
...
//                $appellation="妈妈";
//            }
//            $where['appellation'] = trim($appellation);
//            $rel_res = M("relation_stu_user")->where($where)->select();
//            if ($rel_res) {
//                $ret = format_ret(0, "该关系已经被绑定");
//                echo json_encode($ret);
...
//        //查询用户是否在用户表中
//        $use_res = M("xctuserwechat")->where(array("weixin"=>$weixin,"appid"=>$appid))->find();
//
//        if ($use_res) {
//            //如果存在就拿用户的userid
//            //用户id
//            $userid = $use_res['userid'];
//            //查找用户是否已经绑定这个关系
...
//    }
    ...
    public function Tbuilds(){
        ...
        if(empty($_SESSION["user"]['weixin']) or empty($_SESSION["APPID"]) or empty($_SESSION["manage_id"])){
        //if(empty($_SESSION["user"]['weixin']) or empty($_SESSION["APPID"])){
            $ret = format_ret(0, "页面停留过久,请关闭后重新打开");
            echo json_encode($ret);
            die();
        }
