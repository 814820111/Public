...
class BindingController extends WeixinbaseController
{
    ...
    function accept(){
        ...
        if(I("appid")){
            ...
            if($result){
                ...
                $REDIRECT_URI = "$http/index.php/weixin/Binding/getCode";
    ...
    function getCode(){
        ...
        $http = "http://".$_SERVER['HTTP_HOST'].__ROOT__;
        header("location:".$http."/index.php/weixin/Binding/build");

    }
    public function builds(){
//        $http = "http://".$_SERVER['HTTP_HOST'].__ROOT__;
//        $url = "$http/index.php/weixin/Index/accept?schoolid=2";
//        header("location:$url");
        $this->accept();
    ...
    //家长绑定处理
    /*stu_name 学生名字 phone 手机号码
     * name 家长名字 relation 关系  number 绑定码
     */
    public function buildings()
    {
        if(empty($_SESSION["user"]['weixin']) or empty($_SESSION["APPID"]) or empty($_SESSION["manage_id"])){
        ...
        if($stu_res){
            ...
            $schoollist = M("wxmanage_school")->where(array("manage_id"=>$manage_id,"schoolid"=>$schoolid))->find();
            if(!$schoollist){
                $content="该学校没有开通使用这个公众号的权限";
                $this->Write_Weixin_StuLog($schoolid,$_SESSION["APPID"],$stu_name,$build_number,$name,$relation,$phone,2,$content);
                $ret = format_ret(0, "该学校没有开通使用这个公众号的权限");
                echo json_encode($ret);
                die();
            }
        }
        ...
        if (!$stu_res) {
            ...
            $this->Write_Weixin_StuLog(0,$_SESSION["APPID"],$stu_name,$build_number,$name,$relation,$phone,2,$content);
        ...
        if($relation==1 or $relation==2){
            ...
            if ($rel_res) {
                $content="该关系已经被绑定";
                $this->Write_Weixin_StuLog($stu_res["schoollid"],$_SESSION["APPID"],$stu_name,$build_number,$name,$relation,$phone,2,$content);
        ...
        if ($rets>5){
            $content="绑定名额已经用完";
            $this->Write_Weixin_StuLog($stu_res["schoollid"],$_SESSION["APPID"],$stu_name,$build_number,$name,$relations,$phone,2,$content);
       ...
       // 根据微信和APPID判断该用户否存在
        ...
        if(count($use_res)){
            foreach ($use_res as $value){
                ...
                if ($sereach) {
                    $content="该微信号已经绑定过这个学生!";
                    $this->Write_Weixin_StuLog($stu_res["schoollid"],$_SESSION["APPID"],$stu_name,$build_number,$name,$relations,$phone,2,$content);
                    //如果有就返回说只能绑定一个
                    $ret = format_ret(0, "该微信号已经绑定过这个学生!");
                    echo json_encode($ret);
                    die();
                }
            }
...
//        }
        if(count($users)){
            ...
            if(count($weixinuser) and $weixinuser["weixin"]==$weixin){
                $content="该手机号码已经绑定过其他微信,不能使用,请更换手机号!";
                $this->Write_Weixin_StuLog($stu_res["schoollid"],$appid,$stu_name,$build_number,$name,$relations,$phone,2,$content);
                $ret = format_ret(0, "该手机号码已经绑定过其他微信,不能使用,请更换手机号!");
                echo json_encode($ret);
                die();
            }
        }
        ...
        if(count($wexinuser)){
            //存在,添加学生关系表relation（存在，说明之前用同一个手机号同一个微信号绑定过别的角色）
                    ...
                    if ($rela_add_res) {
                        $content="绑定成功";
                        $this->Write_Weixin_StuLog($stu_res["schoollid"],$appid,$stu_name,$build_number,$name,$relations,$phone,1,$content);
    ...
    public function Tbuilds(){
        ...
        if(empty($result)){
            $content = "不存在这个手机号码";
            $this->Write_Weixin_TeaLog(0,$_SESSION["APPID"],$number,$phone,2,$content);
        ...
        if(empty($query)){
            $content = "绑定码不正确，请联系学校管理员";
            $this->Write_Weixin_TeaLog(0,$_SESSION["APPID"],$number,$phone,2,$content);
            $ret= $this->format_ret(0,"绑定码不正确，请联系学校管理员");
            echo json_encode($ret);
            return;
        }
        $manage_id = $_SESSION["manage_id"];//公众号ID
        $schoollist = M("wxmanage_school")->where(array("manage_id"=>$manage_id,"schoolid"=>$schoolid))->find();
        if(!$schoollist){
            $content = "该学校用户没有开通使用这个公众号的权限";
            $this->Write_Weixin_TeaLog($schoolid,$_SESSION["APPID"],$number,$phone,2,$content);
            $ret = format_ret(0, "该学校用户没有开通使用这个公众号的权限");
            echo json_encode($ret);
            die();
        }
        ...
        $xctuserwechat = M('xctuserwechat')->where(array("userid"=>$userid,"weixin"=>$weixin,"appid"=>$appid))->find();
        if($xctuserwechat){
            $content = "您已经绑定过,无需在绑定";
            $this->Write_Weixin_TeaLog($schoolid,$appid,$number,$phone,2,$content);
            $ret = $this->format_ret(0,"您已经绑定过,无需在绑定");
            echo json_encode($ret);
            return;
        }
        ...
        if($arr){
            $content = "绑定成功";
            $this->Write_Weixin_TeaLog($schoolid,$appid,$number,$phone,1,$content);
    ...
    //教师绑定日志写入
    public function Write_Weixin_TeaLog($schoolid,$appid,$tch_bindingkey,$tch_phone,$status,$content)
    {
        $log_data["schoolid"] =   $schoolid;
        $log_data["bindingtype"] =  2;
        $log_data["appid"] =  $appid;
        $log_data["tch_bindingkey"] =  $tch_bindingkey;
        $log_data["tch_phone"] =  $tch_phone;
        $log_data["status"] =  $status;
        $log_data["content"] =  $content;
        $log_data["binddate"] =  date("Y-m-d",time());
        $log_data["bind_datetime"] =  date("Y-m-d H:i:s",time());
