...
class ChatController extends WeixinbaseController
{
   ...
   public function index()
   {
     //根据userid  获取huanxinid   todo得加上schoolid  teacherid  来区分，这个先稍等以后的问题
       ...
       $type = !$_SESSION['studentid']?1:0;


       $chat_list  = $this->xcGetChatListData($userid,$type);
   ...
   public function chat()
   {
      ...
      //获取对象聊天
       ...
       //获取聊天记录
       $result = $this->xcGetChatData($send_uid,$receive_uid,'',$receiver_type,$send_type,$receive_type);
   ...
   //获取聊天对象名称
    ...
    //拉取聊天信息
    public function xcGetChatData($send_uid,$receive_uid,$index,$receiver_type,$send_type,$receive_type){
...
//        $receiver_type=intval(I('type'));
//        $index = I('index');
        $res = $this->getChatData($send_uid,$receive_uid,$index,$receiver_type,$send_type,$receive_type);
    ...
    //获取聊天信息
    function getChatData($uid, $chat_uid, $index,$type=0,$send_type,$receive_type){
        ...
        if($type==1){
            ...
            $where='(send_uid='.$uid .' and send_type = '.$send_type.' and recive_type ='.$receive_type.' and receive_uid = '.$chat_uid.') or (send_uid='.$chat_uid .' and receive_uid = '.$uid.' and send_type = '.$receive_type.' and recive_type = '.$send_type.')';
    ...
    public function xcGetChatListData($uid,$type){
        $Message =  A('Apps/Message');
//        $res = $Message->getChatListData($uid);
        $res = $this->getChatListData($uid,$type);
    ...
    function getChatListData($uid,$type){
        $data = array();
        $where = array(
            'uid'=>$uid,
            'send_type'=>$type,
...
//                $this->updateChatList($uid, $chat_uid, $content, $lastChatId,$send_type,$recive_type,$type);
                 $this->updateChatList($chat_uid, $uid, $content, $lastChatId,$type,$send_type,$recive_type);
    ...
    function updateChatList($chatUid,$uid,$content,$lastChatId,$type,$send_type,$recive_type){
       if($type==1)
       {
           $query = M('chat_list')->where(array('uid'=>$uid, 'chat_uid'=>$chatUid))->order('create_time desc')->find();
       }else{
           $query = M('chat_list')->where(array('uid'=>$uid, 'chat_uid'=>$chatUid,"send_type"=>$send_type,"receive_type"=>$recive_type))->order('create_time desc')->find();
       }
...
;
   ...
   //检查是否已经存在
    ...
    public function is_recive()
    {
        ...
        //根据
        $send_id = $this->wxtuser->where(array("huanxinid"=>$send_hx))->getField("id");


        $message = $this->chat->where(array("send_uid"=>$send_id,"receive_uid"=>$receive_uid,"recive_type"=>$type))->find();

        if($message)
        {
            $res = $this->format_ret(1,$message);
        }else{
            $res = $this->format_ret(0,'不是该端的消息');
        }
       echo json_encode($res);
    }
