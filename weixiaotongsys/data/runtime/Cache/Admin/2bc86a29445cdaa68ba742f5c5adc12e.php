...
<body class="J_scroll_fixed">
	...
	<div class="wrap J_check_wrap">
		<ul class="nav nav-tabs">
			<li><a href="<?php echo U('index');?>">绑定码绑定查询</a></li>
			<li  class="active"><a href="<?php echo U('code_index');?>">手机验证码绑定查询</a></li>
		...
		<form class="well form-search" method="get" action="<?php echo U('code_index');?>">
			<div class="search_type cc mb10">
				<div class="mb10">
					<span class="mr20">
						公众号：
					        <select name="appid" class="normal_select" id="wx_select">
								<option value="0">请选择</option>
                                <?php if(is_array($wxmanage)): foreach($wxmanage as $key=>$v): ?><!--<?php $navcid_selected=$navcid==$vo['navcid']?"selected":""; ?>-->
									<!--<option value="<?php echo ($vo["navcid"]); ?>" <?php echo ($navcid_selected); ?>><?php echo ($vo["name"]); ?></option>-->
                                    <option value="<?php echo ($v["wx_appid"]); ?>" <?php if($appid==$v["wx_appid"]){ echo 'selected';} ?>><?php echo ($v["wx_name"]); ?></option><?php endforeach; endif; ?>
                                <?php echo ($nav_trees); ?>
                            </select>
					<!--省份选择：-->
						<!--<select  class="province"  name="province" id="province" style="width: auto;">-->
							<!--<option value="">省级选择</option>-->
							<!--<?php if(is_array($Province)): foreach($Province as $key=>$vo): ?>-->
							   <!--<?php $pro=$province==$vo['term_id']?"selected":""; ?>-->
								<!--<option value="<?php echo ($vo["term_id"]); ?>"<?php echo ($pro); ?>><?php echo ($vo["name"]); ?></option>-->
					   ...
					   <!--</select> &nbsp;&nbsp;-->
						<select class="mohu" name="tiaojian">
							...
							<option value="code" <?php if($tiaojian==code) echo("selected");?>>验证码</option>
						   </select>
					...
					<select class="select_1" name="status" id="cis1" style="width: auto;">
                        ...
                        <a class="btn btn-danger" href="<?php echo U('code_index');?>">清空</a>
		...
		<form class="J_ajaxForm" action="" method="post">
			...
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($result)): foreach($result as $key=>$vo): ?><tr>
						...
						<!--<td>-->
							<!--<?php if($vo['bindingtype'] == 1): ?>-->
							<!--家长-->
							<!--<?php endif; ?>-->
							<!--<?php if($vo['bindingtype'] == 2): ?>-->
							<!--老师-->
							<!--<?php endif; ?>-->
							<!--</td>-->
        ...
        $(".select_3").change(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    $(".Sio").text("")
                    var html = "";
                    html += '<option class="Sio" value="0">全部</option> '
                    res = eval(res.data);
                    for(var i = 0; i < res.length; i++) {
                        var school_name = res[i].school_name; //学校的名字
                        var schoolid = res[i].schoolid; //学校的ID
