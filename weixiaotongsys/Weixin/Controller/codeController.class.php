...
session_start();
header("Content-type:text/html;charset=utf-8");
class codeController extends WeixinbaseController{
    function code()
    {
        $schoolid = I("qycode");
        ...
        $data['name'] = $_SESSION["user"]['nickname'];
        $data['schoolid'] = $schoolid;
        $res = M("wxtuser")->save($data);
        if ($res){
            echo "成功";
        }else{
            echo "失败";
...
//用户同意之后就获取code  通过获取code可以获取一切东西了  机智如我
    function getCode(){
        ...
        $http = "http://".$_SERVER['HTTP_HOST'].__ROOT__;
        header("location:".$http."/index.php/weixin/index/code/code?qycode".$qycode);
    }
